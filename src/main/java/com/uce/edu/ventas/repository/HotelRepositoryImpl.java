...
public class HotelRepositoryImpl implements IHotelRepository {
	...
	public List<Hotel> seleccionarHabitacionesInnerJoin(String clase) {
		...
		TypedQuery<Hotel> myQuery = this.entityManager
				.createQuery("SELECT h FROM Hotel h JOIN h.habitaciones d WHERE h.tipo = :tipo", Hotel.class);
	...
	public List<Hotel> seleccionarHabitacionesRightJoin(String tipo) {
		// TODO Auto-generated method stub
		TypedQuery<Hotel> myQuery = this.entityManager
				.createQuery("SELECT h FROM Hotel h RIGHT JOIN h.habitaciones d WHERE h.nombre = :tipo", Hotel.class);
	...
	public List<Hotel> seleccionarHabitacionesLeftJoin(String tipo) {
		// TODO Auto-generated method stub
		TypedQuery<Hotel> myQuery = this.entityManager
				.createQuery("SELECT h FROM Hotel h LEFT JOIN h.habitaciones d WHERE h.tipo = :tipo", Hotel.class);
		myQuery.setParameter("tipo", tipo);
		List<Hotel> lista = myQuery.getResultList();
		for (Hotel f : lista) {
			f.getHabitaciones().size();
	...
	public List<Hotel> seleccionarHabitacionesFullJoin(String tipo) {
		// TODO Auto-generated method stub
		TypedQuery<Hotel> myQuery = this.entityManager
				.createQuery("SELECT h FROM Hotel h FULL JOIN h.habitaciones d WHERE d.clase = :tipo", Hotel.class);
		myQuery.setParameter("tipo", tipo);
		List<Hotel> lista = myQuery.getResultList();
		for (Hotel f : lista) {
			f.getHabitaciones().size();
	...
	public List<Hotel> seleccionarHabitacionFetchJoin() {
		...
		TypedQuery<Hotel> myQuery = this.entityManager
				.createQuery("SELECT h FROM Hotel h JOIN FETCH h.habitaciones d", Hotel.class);
